import polars as pl
from src.lib.pipeline import df
...
def email_filtering_tool(
    ...
    Args:
        ...
        sort_by (str, optional): Column to sort the results by. Default is 'date_dt'. Use 'date_dt' for chronological sorting.
    ...
    if range_start:
        mask = mask & (pl.col("date_dt") >= range_start)
    if range_end:
        mask = mask & (pl.col("date_dt") <= range_end)
    ...
    if labels:
        ...
        labels_mask = pl.col("labels_normalized").map_elements(
            lambda email_labels: any(
                lbl in email_labels.split(",") for lbl in labels
            ),
            return_dtype=bool,
        )
